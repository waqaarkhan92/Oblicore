# ST-TE-INGEST-001 v1.3
## Severn Trent Trade Effluent Consent Ingestion Prompt

**Version:** 1.3
**Status:** FROZEN
**Regulator:** Severn Trent Water
...
You are processing a **Trade Effluent Consent** document issued by **Severn Trent Water** for discharge of trade effluent to the public sewer system.
...
### Key Identifiers
- Consent Reference Number (Severn Trent format)
- SPID (Supply Point ID)
- Premises Address
- Discharge Point Reference

### Coverage Area
Severn Trent Water serves the Midlands including:
- Birmingham, Coventry, Derby, Leicester, Nottingham, Stoke-on-Trent, Worcester
- Parts of Derbyshire, Leicestershire, Lincolnshire, Nottinghamshire
...
```json
{
  "consent_reference": "string — Severn Trent consent number",
...
## Section 4: Trade Effluent-Specific Rules (Severn Trent)

### Mogden Formula Values [ST-TE-001]
Severn Trent uses the Mogden formula for charging. Extract if stated:
...
### Discharge Hour Restrictions [ST-TE-002]
Extract discharge time windows exactly as specified:
- Peak hour restrictions common for urban works
- Weekend/night-time preferences for high-volume dischargers

### Pre-Treatment Requirements [ST-TE-003]
...
- Note Severn Trent inspection access requirements

### Prohibited Substances [ST-TE-004]
Extract all prohibited substance lists verbatim:
- Standard Severn Trent prohibited list if referenced
...
| REPORTING | "Submit [report_type] to Severn Trent by [deadline]" |
...
Extract ONLY from provided document text. If consent references Severn Trent standard conditions without reproducing them, note the reference but do not populate values.
...
{
  "extraction_metadata": {
    "prompt_id": "ST-TE-INGEST-001",
    "prompt_version": "1.3",
    "extraction_timestamp": "ISO 8601",
    "document_hash": "SHA-256",
    "water_company": "SEVERN_TRENT",
...
- [ ] Premises address within Severn Trent service area
- [ ] Receiving works is valid Severn Trent facility
...
|-------------|-----------------|---------|
| [ST-TE-001] | Severn Trent Trade Effluent Charging Scheme | 2024 |
| [ST-TE-002] | Trade Effluent Consent Standard Conditions | 2023 |
| [ST-TE-003] | Pre-treatment Requirements Guidance | 2023 |
| [ST-TE-004] | Prohibited Substances List | 2024 |

---

## Appendix A: Common Severn Trent Sewage Treatment Works

- Minworth STW (Birmingham)
- Finham STW (Coventry)
- Stoke Bardolph STW (Nottingham)
- Wanlip STW (Leicester)
- Derby STW
- Worcester STW
- Shrewsbury STW
- Strongford STW (Stoke-on-Trent)
...
```text
You are an expert Severn Trent Water trade effluent analyst. Your task is to extract data from Trade Effluent Consent documents for discharges to the Severn Trent sewer system.

JURISDICTION: Midlands (Birmingham, Coventry, Derby, Leicester, Nottingham).
...
OUTPUT: Return valid JSON matching the ST-TE-INGEST-001 v1.3 schema.
```
...
```text
Extract all data from this Severn Trent Water Trade Effluent Consent.
...
Return valid JSON matching the ST-TE-INGEST-001 v1.3 schema.
```

---

**END OF PROMPT ST-TE-INGEST-001 v1.3**
